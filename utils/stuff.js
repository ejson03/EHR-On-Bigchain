...
const createAccess = async (
  ...
  for (index in dlist) {
    ...
    data = {
      email: doctorEmail,
      key: cryptoService.encryptRSA(
        secretKey,
        getRSAKey(doctorEmail, "doctor")
      ),
    };
...
const showAccess = async (demail, records) => {
  ...
  for (const asset of records) {
    transaction = await bigchainService.listTransactions(asset.id);
...
const getDoctorFiles = async (email) => {
  let metadata = bigchainService.getMetadata(email);
  let data = [];
  let assetlist = new Set();

  for (const meta of metadata) {
    tx = await bigchainService.listTransactions(meta.id);
  ...
  for (const asset of assetlist) {
    tx = await bigchainService.listTransactions(asset);
